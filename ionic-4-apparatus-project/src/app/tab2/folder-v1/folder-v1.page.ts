...
@Component({
  selector: 'app-folder-v1',
  templateUrl: './folder-v1.page.html',
  styleUrls: ['./folder-v1.page.scss'],
})
export class FolderV1Page implements OnInit {
  directories = [];
  folder = '';
  copyFile: Entry = null;
  shouldMove = false;
...
async itemClicked(file: Entry) {
  if (this.copyFile) {
    ...
    if (file.isFile) {
      ...
      //this.router.navigateByUrl(`/folder-v1/${folder}`);
      this.router.navigateByUrl(`/tabs/tabs/tab2/folder-v1/${folder}`);
      //this.router.navigateByUrl(`/tabs/tab2/folder-v1/${folder}`);
      //this.router.navigateByUrl(`/folder-v1/${folder}`);
      //this.navCtrl.navigateForward(`/tabs/tabs/tab2/folder-v1/${folder}`);
    }
  }
}
